        ...
        for i in np.arange(1, 11, 1):
            ...
            for SP_value in sp_values:
                ...
                if cumulative_profit_loss.empty:
                    cumulative_profit_loss = profit_loss_df[['date', 'change', 'num_trades']]
                else:
                    temp_df = pd.merge(cumulative_profit_loss, profit_loss_df[['date', 'change', 'num_trades']], on='date',
                                       how='outer', suffixes=('', '_new'))
                    cumulative_profit_loss = pd.DataFrame({
                        'date': temp_df['date'],
                        'change': temp_df['change'].ffill().add(temp_df['change_new'].ffill(), fill_value=0),
            ...
            cumulative_profit_loss.to_csv(os.path.join(save_path, f'cumulative_profit_loss_cost_{trans_costs}_{avg_periods}.csv'),
                                          index=False)
            trades.to_csv(os.path.join(save_path, f'all_trades_cost_{trans_costs}_{avg_periods}.csv'), index=False)
            trade_pnl.to_csv(os.path.join(save_path, f'trade_pnl_cost_{trans_costs}_{avg_periods}.csv'), index=False)
            print(f'saved profit and loss as cumulative_profit_loss_cost_{trans_costs}_{avg_periods}.csv')
            print(f'saved trades as all_trades_cost_{trans_costs}_{avg_periods}.csv')
